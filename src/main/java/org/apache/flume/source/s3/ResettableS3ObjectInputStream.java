...
public class ResettableS3ObjectInputStream extends ResettableInputStream
   ...
   */
  public ResettableS3ObjectInputStream(S3Object file, PositionTracker tracker)
  ...
  public ResettableS3ObjectInputStream(S3Object file, PositionTracker tracker,
    ...
    this.in = file.getDataInputStream();
    this.chan = Channels.newChannel(this.in);
    ...
    charBuf.flip();
    this.fileSize = file.getContentLength();
  ...
  private void refillBuf() throws IOException {
    buf.compact();
    // TODO critical to fix this
//    chan.skip(position); // ensure we read from the proper offset
    chan.read(buf);
    buf.flip();
  }
